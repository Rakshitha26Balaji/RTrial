...
export default function DomesticLeadsForm({ OrderData }) {
  ...
  /** ⭐ Field Styling (same as TenderForm) */
  const commonFieldSx = {
    minWidth: "270px",
    "& .MuiInputBase-root": {
      height: 58,
  ...
  const onSubmit = (data) => {
    console.log("Domestic Lead Submitted:", data);
  ...
  const handleDownloadJSON = () => {
    ...
    link.download = "Domestic_Lead_Data.json";
    link.click();
  };
          ...
          >
            <Box sx={{ textAlign: "center", mb: 3 }}>
              ...
              <Typography variant="body1" sx={{ color: "#37474F" }}>
                Kindly fill all required fields to create a domestic lead record
              </Typography>
            </Box>

            <form onSubmit={handleSubmit(onSubmit)}>
              {/* SECTION 1 */}
              <Card sx={{ mb: 3, borderRadius: 3, background: "linear-gradient(135deg, #e3f2fd 0%, #ffffff 100%)" }}>
                <CardContent>
        ...
        {value === 1 && OrderData && (
          <Paper elevation={4} sx={{ p: 4, borderRadius: 4 }}>
            ...
            <Typography variant="body1" sx={{ color: "#455A64" }}>
              (Later you can integrate GET API to show domestic leads here.)
            </Typography>
          </Paper>
        )}
      </Box>
